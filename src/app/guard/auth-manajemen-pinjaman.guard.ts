...
@Injectable()
export class AuthManajemenPinjamanGuard implements CanActivate {
  constructor(
    ...
    return new Observable<boolean>(observer => {
      if (this.authService.isSignedIn()) {
        ...
        if (parselocalstorage.role !== 'Admin Peers' ){
            if(parselocalstorage.access.mn_management_pinjaman === 1 || parselocalstorage.access === 'all')observer.next(true);
            else {
                observer.next(false); 
