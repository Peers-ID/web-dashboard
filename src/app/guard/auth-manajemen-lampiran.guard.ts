...
@Injectable()
export class AuthManajemenLaporanGuard implements CanActivate {
  constructor(
    ...
    return new Observable<boolean>(observer => {
      if (this.authService.isSignedIn()) {
        ...
        if (parselocalstorage.role !== 'Admin Peers' ){
            if(parselocalstorage.access.mn_management_laporan === 1 || parselocalstorage.access === 'all')observer.next(true);
            else {
                observer.next(false); 
