...
@Injectable()
export class AuthPengaturanParameterGuard implements CanActivate {
  constructor(
    ...
    return new Observable<boolean>(observer => {
      if (this.authService.isSignedIn()) {
        ...
        if (parselocalstorage.role !== 'Admin Peers'){
            if( parselocalstorage.access.mn_pengaturan_pinjaman === 1 || parselocalstorage.access === 'all')observer.next(true);
            else {
                observer.next(false); 
