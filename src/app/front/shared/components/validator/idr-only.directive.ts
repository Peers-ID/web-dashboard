...
export class IdrOnlyDirective implements OnInit {
  currencyChars = new RegExp('[\.,]', 'g'); 
  // we're going to remove commas and dots

  constructor(public el: ElementRef, public renderer: Renderer2, private decimalPipe: DecimalPipe, private cp: CurrencyPipe) {}
  ...
  format(val:string) {
    ...
    // const numberFormat = this.currencyChars;
    // console.log(numberFormat); // raw number

    // 2. format the number (add commas)
    const Rp = this.decimalPipe.transform(numberFormat, '1.0-0', 'id-IDR');
    // const Rp = this.cp.transform(numberFormat, 'USD', 'symbol', '1.2-2' );

    // 3. replace the input value with formatted numbers
    this.renderer.setProperty(this.el.nativeElement, 'value', Rp);
  }
